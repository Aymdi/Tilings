...
void deck_init(struct deck* d){
	...
	d->size = 4;//Fill the deck with 4 types of tiles

    //Tile 1
	deck_tile[0] = (struct tile) {{color_from_name("vert"),
					               color_from_name("noir"),
					               color_from_name("bleu"),
					               color_from_name("noir")}};
	d->cards[0] = (struct deck_pair) {&deck_tile[0],4};

	//Tile 2
	deck_tile[1] = (struct tile) {{color_from_name("noir"),
					               color_from_name("vert"),
					               color_from_name("noir"),
					               color_from_name("bleu")}};
	d->cards[1] = (struct deck_pair) {&deck_tile[1],4};

	//Tile 3
	deck_tile[2] = (struct tile) {{color_from_name("bleu"),
					               color_from_name("vert"),
					               color_from_name("bleu"),
					               color_from_name("rouge")}};
    d->cards[2] = (struct deck_pair) {&deck_tile[2],2};
	...
	deck_tile[3] = (struct tile) {{color_from_name("noir"),
					               ...
					               color_from_name("rouge"),
					               color_from_name("bleu")}};
	d->cards[3] = (struct deck_pair) {&deck_tile[3],2};
